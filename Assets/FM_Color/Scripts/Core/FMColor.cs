...
namespace FMCOLOR
{
    ...
    public class FMColor : MonoBehaviour
    {
        ...
        private bool beginCameraRendering = false;
        private bool foundOutputTexture = false; //fixed Render Texture Output issue
        private RenderTexture outputRenderTexture; //fixed Render Texture Output issue
        ...
        private void RenderPipelineManager_beginCameraRendering(ScriptableRenderContext context, Camera camera)
        {
            //OnPreRender();
            if (Cam == null) return;
            beginCameraRendering = true;
//#if UNITY_EDITOR
            currentRenderTexture = RenderTexture.active;
//#endif

            //fixed Render Texture Output issue
            if (Cam.targetTexture != null)
            {
                outputRenderTexture = Cam.targetTexture;
                foundOutputTexture = true;
            }
        ...
        {
            //OnPostRender();
            if (!beginCameraRendering) return;
            beginCameraRendering = false;
    ...
    //#endif
        ...
        private void FMCOLOR_Blit()
        {
            ...
            if (FXAASettings.Enable)
            {
                ...
                Graphics.Blit(MainTex, foundOutputTexture ? outputRenderTexture : null as RenderTexture, FXAAMat);
                RenderTexture.ReleaseTemporary(MainTex);
            }
            else
            {
                Graphics.Blit(rt, foundOutputTexture ? outputRenderTexture : null as RenderTexture, material, pass);
            }

            //fixed Render Texture Output issue
            if (foundOutputTexture) Cam.targetTexture = outputRenderTexture;
        }
        ...
        private void CheckResolution()
        {
            ...
            //fixed Render Texture Output issue
            if (foundOutputTexture)
            {
                if (outputRenderTexture.format != RenderTextureFormat.ARGB32)
                {
                    outputRenderTexture.format = RenderTextureFormat.ARGB32;
                }
            }
        }
#else
